...
9.4
突然开始思考，这个项目是否真的能对我派上用场，主要是日本科技公司主要还是写各种式样书，那我的学习目标是否应该转变为把这个项目完成后再加一堆设计和式样书，包括详细设计，基础设计等等内容，这样面试的时候才能符合日本人的习惯。不过最基本的一点我还是能做到，用一个html来编写我的简历，最起码能展示一定的编码能力吧

9.10 
发现时间处理这块其实是一个非常大的工程，最好的方案就是前端统一使用utc时间，然后后端存储也是utc时间，最后返回给前端的时候再转为本地时间，这样就能避免各种时区问题了，此外还要注意夏令时的问题，这个就比较麻烦了，不过大部分国家都没有这个问题。这个小细节对项目来说应该是比较好的一个经验，因为毕竟需要从整个架构上动手了（哭）。不过这个确实可以在简历中是一个亮点，毕竟算是实际开发经验。  
